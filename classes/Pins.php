...
class Pins{
	...
	public function generate_scratch_pins($data){
		...
		if (empty($q) || empty($p) || empty($t)) {
			// show error
	$this->response =$this->alert->alert_toastr("error","Invalid submission, Please try again!","GSSOTA SAYS");
		}elseif ($q > 5) {
			// code...
	$this->response =$this->alert->alert_toastr("error","Maximum Pin to generate is 5, Please try again!","GSSOTA SAYS");
		}elseif (!$this->config->check_user_activity_allowed("card_generator")) {
		$this->response =$this->alert->alert_toastr("error","Card Generator is Disabled, Kindly contact your Administrator!","GSSOTA SAYS");
		} 
		else{
 ...
 if ($cardModuleStatus == 0) {
 	// code...
 	$this->response =$this->alert->alert_toastr("error","Oops! PIN Creation is not allowed at the Moment!","GSSOTA SAYS");
 	}else{
 	switch ($t) {
 		case 'rp':
   ...
   $this->query ="INSERT INTO $this->_table_ (pin_code,pin_serial,pin_desc,price,created_at) VALUES(?,?,?,?,?)";
    ...
    if ($this->stmt!=NULL) {
    	// code...
    $this->response =$this->alert->alert_toastr("success","You have Successfully Generated <b>".$countInserted." ".$this->_pin_desc_."","GSSOTA SAYS")."<script>setTimeout(()=>{window.location.reload();},2000)</script>";
    }else{
    $this->response = $this->alert->alert_toastr("error",$lang['server_error'],"GSSOTA SAYS");
    }
 }
		}
...
public function get_pins($table){
	...
	public function delete($table,$id){
		if (!empty($table) && ! empty($id)) {
			...
			if ($this->stmt->rowCount()>0) {
		$this->response = $this->alert->alert_toastr("success","Pin Deleted Successfully!","GSSOTA SAYS")."<script>setTimeout(()=>{window.location.reload();},2000)</script>";
			}else{
		 $this->response = $this->alert_toastr("error",$lang['server_error'],"GSSOTA SAYS");
			}
		}
		return $this->response;
	}
