	...
	public function create_subject($data){
		...
		if ($this->config->isEmptyStr($bypass) || $bypass!= md5("oiza1")) {
			...
			if ($this->stmt->rowCount()==1) {
				...
				try {
					$teacher = implode(",", $data['subjectTeacher']);
					$this->dbh->beginTransaction();
...
return $diff->format('%y');
  ...
  public function isClassTeacher($staffId){
  	//return true;
  		$this->stmt = $this->dbh->prepare("SELECT * FROM `visap_staff_tbl` WHERE staffId=? AND staffRole=? AND staffGrade!=NULL");
...
//filter attendance student by date
	public function show_attendance_by_date($student_class,$rollType,$date){
		if (!$this->config->isEmptyStr($student_class)) {
			$_date = date("Y-m-d",strtotime($date));
			$this->stmt =$this->dbh->prepare("SELECT * FROM `visap_class_attendance_tbl` WHERE studentGrade=? AND roll_call=? AND attendant_date=? ORDER BY stdReg ASC");
			//execute
			$this->stmt->execute(array($student_class,$rollType,$_date));
			if ($this->stmt->rowCount()>0) {
			$this->response = $this->stmt->fetchAll();
			}else{
				$this->response = false;
...
//
  ...
  public function set_new_office($data){
  	$office = $this->config->Clean($data['office']);
		...
		if ($this->config->isEmptyStr($bypass) || $bypass!= md5("oiza1")) {
			// code...
			$this->response = $this->alert->alert_toastr("error","Authentication Failed, Please Check your Connection and Try again!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($office) || $this->config->isEmptyStr($office_status)) {
			$this->response = $this->alert->alert_toastr("warning","Office Description and Status is required!","GSSOTA SAYS");
		}else{
			//check for duplicate Class name
			...
			if ($this->stmt->rowCount()==1) {
				// code...
				$this->response = $this->alert->alert_toastr("error","$office already Exists!","GSSOTA SAYS");
			}else{
				try {
					$this->dbh->beginTransaction();
					//create the new Classroom
				...
				if ($this->stmt->execute([$office,$office_status,$date])) {
					// code...
					 $this->dbh->commit();
			$this->response = $this->alert->alert_toastr("success",$office." Office Added Successfully","GSSOTA SAYS")."<script>setTimeout(()=>{
			window.location.reload();
			},2500);</script>";
				}
			...
			$this->dbh->rollback();
    $this->response  =$this->alert->alert_msg("error","Failed: Error Occurred: ".$e->getMessage(),"GSSOTA SAYS");
				}
			}
		}
		return $this->response;
...
// TO BE UPLOADED TO SERVER
  ...
  public function update_school_profile_details($data){
  	...
  	if ($this->config->isEmptyStr($school_name) ||$this->config->isEmptyStr($slogan) || $this->config->isEmptyStr($approval_number) || $this->config->isEmptyStr($school_history) || $this->config->isEmptyStr($founded_year) || $this->config->isEmptyStr($school_address)|| $this->config->isEmptyStr($school_state) || $this->config->isEmptyStr($school_city) || $this->config->isEmptyStr($country) || $this->config->isEmptyStr($default_language) || $this->config->isEmptyStr($school_phone) || $this->config->isEmptyStr($school_fax) || $this->config->isEmptyStr($school_email) || $this->config->isEmptyStr($school_gmail) || $this->config->isEmptyStr($website_link)) {
  		$this->response = $this->alert->alert_toastr("error","Invalid Form submission","GSSOTA SAYS");
  	}elseif ($this->config->isEmptyStr($auth_pass)) {
  	$this->response = $this->alert->alert_toastr("error","Enter an Authentication Passcode to Continue","GSSOTA SAYS");
  	}elseif ($auth_pass !== __OSO__CONTROL__KEY__) {
  	$this->response = $this->alert->alert_toastr("error","Invalid Authentication Entered","GSSOTA SAYS");
  }else{
  	$id =1;
  	//update action now
  	try {
					...
					//create the new Classroom
				$date = date("Y-m-d");
				$this->stmt = $this->dbh->prepare("UPDATE `visap_school_profile` SET school_name=?,govt_approve_number=?,school_address=?,school_slogan=?,school_state=?,school_city=?,country=?,school_email=?,school_phone=?,school_fax=?,website_name=?,default_language=?,school_history=? WHERE id=? LIMIT 1");
				if ($this->stmt->execute(array($school_name,$approval_number,$school_address,$slogan,$school_state,$school_city,$country,$school_email,$school_phone,$school_fax,$website_link,$default_language,$school_history,$id))) {
					// code...
					 $this->dbh->commit();
...
public function upload_affective_domain($data){
	...
	$this->response = $this->alert->alert_toastr("warning","Invalid form Submission! ","GSSOTA SAYS");
			}elseif ($auth_pass !== __OSO__CONTROL__KEY__) {
	$this->response = $this->alert->alert_toastr("error","Invalid Authentication Code! ","GSSOTA SAYS");
			}else{
for ($i=0; $i < (int)$total_count; $i++) { 
			...
			if ($this->stmt->rowCount()>0) {
			$this->response = $this->alert->alert_toastr("warning","Affective Domain for $term $session is already uploaded for ".strtoupper($std_class)."! ","GSSOTA SAYS");
			}else{
				try {
					$this->dbh->beginTransaction();
					...
					$this->dbh->commit();
				$this->response = $this->alert->alert_toastr("success","Affective Domain Uploaded Successfully, Please wait... ","GSSOTA SAYS")."<script>setTimeout(()=>{
				...
				$this->dbh->rollback();
					$this->response  = $this->alert->alert_toastr("error","Uploading Failed! Please try again...: Error: ".$e->getMessage(),"GSSOTA SAYS");
					}
			}
		}
			}
...
public function add_new_student_office_title($data){
	...
	if ($this->config->isEmptyStr($office_name)) {
		$this->response = $this->alert->alert_toastr("warning","Please enter the Office Description to continue!","GSSOTA SAYS");
	}elseif ($this->config->isEmptyStr($office_status)) {
	$this->response = $this->alert->alert_toastr("warning","Please set the status of the new Office to continue!","GSSOTA SAYS");
	}else{
		...
		if ($this->stmt->rowCount() ==1) {
	$this->response = $this->alert->alert_toastr("warning","$office_name already Exists!","GSSOTA SAYS");
		}else{
			$date = date("Y-m-d");
try {
					$this->dbh->beginTransaction();
					...
					$this->dbh->commit();
				$this->response = $this->alert->alert_toastr("success","$office_name Added Successfully, Please wait... ","GSSOTA SAYS")."<script>setTimeout(()=>{
				window.location.reload();
			},1500);</script>";
					}
...
public function update_prefect_office_title($data){
	...
	$this->response = $this->alert->alert_toastr("warning","Please set the status of this Office to continue!","GSSOTA SAYS");
	}else{
		try {
					$this->dbh->beginTransaction();
				...
				if ($this->stmt->execute([$office_name,$office_status,$officeId])) {
					// code...
					 $this->dbh->commit();
			$this->response = $this->alert->alert_toastr("success"," Office Title updated Successfully","GSSOTA SAYS")."<script>setTimeout(()=>{
			window.location.reload();
			},1500);</script>";
				}
