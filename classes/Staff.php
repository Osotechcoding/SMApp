...
class Staff {
	...
	public function login_staff($data){
		...
		$this->response =$this->alert->alert_toastr("error","Login is currently not allowed!","WARNING");
			...
			// code...
			$this->response =$this->alert->alert_toastr("error","Login Details are required!","ERROR");
		}elseif (! $this->config->is_Valid_Email($email)) {
			// code...
			$this->response =$this->alert->alert_toastr("error","Invalid Email address!","ERROR");
		}else{
      ...
      if ($this->config->check_two_passwords_hash($password,$db_password)) {
       ...
       $this->response = $this->alert->alert_toastr("success",$lang['login_success'],$lang['alert-title-success'])."<script>setTimeout(()=>{
         window.location.href='".$urlLink."';
      	...
      	$this->response = $this->alert->alert_toastr("error","Something went wrong!",$lang['alert-title-error']);//Something went wrong
      		}
...
$this->response = $this->alert->alert_toastr("error",$lang['login_error5'],$lang['alert-title-error']);// Email Address Not Found or User Details not found
	...
	public function update_staff_ById($id){}
	...
	public function create_new_staff($d){
		...
		if ($this->config->isEmptyStr($surName)) {
			$this->response = $this->alert->alert_toastr("warning","The staff Surname is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($firstName)) {
	$this->response = $this->alert->alert_toastr("warning","The staff First Name is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($middleName)) {
		$this->response = $this->alert->alert_toastr("warning","The staff Middle Name is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($email)) {
	$this->response = $this->alert->alert_toastr("warning","The staff Portal E-mail is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($mphone)) {
		$this->response = $this->alert->alert_toastr("warning","The staff Phone Number is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($musername)) {
		$this->response = $this->alert->alert_toastr("warning","The staff Portal Username is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($mpassword)) {
		$this->response = $this->alert->alert_toastr("warning","The staff Portal Password is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($education)) {
		$this->response = $this->alert->alert_toastr("warning","The staff Qualification is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($jobType)) {
			$this->response = $this->alert->alert_toastr("warning","The staff Job Type is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($staff_gender)) {
			$this->response = $this->alert->alert_toastr("warning","The staff Gender is Required!","GSSOTA SAYS");
		}elseif ($this->config->isEmptyStr($auth)) {
			$this->response = $this->alert->alert_toastr("warning","Authentication Key is Required!","GSSOTA SAYS");
		}elseif (!$this->config->is_Valid_Email($email)) {
			$this->response = $this->alert->alert_toastr("warning","A valid e-mail address is Required!","GSSOTA SAYS");
		}
		else{
			if ($auth !== __OSO__CONTROL__KEY__) {
				$this->response = $this->alert->alert_toastr("error","Invalid Authentication Key!","GSSOTA SAYS");
			}else{
				//lets check if the email entered is already exists
				if ($this->config->check_single_data('visap_staff_tbl','staffEmail',$email)) {
				$this->response = $this->alert->alert_toastr("warning","$email is already taken, Please try another email address!","GSSOTA SAYS");
				}elseif ($this->config->check_single_data('visap_student_tbl','stdEmail',$email)) {
	$this->response = $this->alert->alert_toastr("warning","$email is already taken on this Portal, Please try another!","GSSOTA SAYS");
				}else{
			 ...
			 try {
			 	...
			 	if ($this->stmt->execute(array($staffRegNo,$fullName,$surName,$email,$hashed_password,$musername,$education,$mphone,$confirmation_code,$staff_gender,$portal_email,$staff_status,$jobType,$reg_date))) {
		 				$this->dbh->commit();
						$this->response = $this->alert->alert_toastr("success","$fullName Registered Successfully","GSSOTA SAYS")."<script>setTimeout(()=>{
							window.location.reload();
						},2500);</script>";
			 	}else{
   ...
   $this->response  =$this->alert->alert_msg("Error Occurred: ".$e->getMessage(),"danger"); 	
    ...
    if ($row = $this->stmt->fetch()) {
      ...
      $value2 = substr($value2, 8,11);//separating numeric part
...
public function assign_staff_office($data){
	...
	$this->response = $this->alert->alert_toastr("error","Select the staff name you want to assign to Office","GSSOTA SAYS");
	}elseif ($this->config->isEmptyStr($office_role)) {
	$this->response = $this->alert->alert_toastr("error","Select the Office you want to assign","GSSOTA SAYS");
	}elseif ($this->config->isEmptyStr($auth_pass)) {
	$this->response = $this->alert->alert_toastr("error","Enter an Authentication Password","GSSOTA SAYS");
		...
		$this->stmt->execute(array($staffId));
		if ($this->stmt->rowCount() ==1) {
			$this->response = $this->alert->alert_toastr("error","The Selected staff is already assigned to an Office","GSSOTA SAYS");
		}else{
			 try {
			 	 $this->dbh->beginTransaction();
			 	...
			 	if ($this->stmt->execute(array($staffId,$office_role))) {
			 		...
			 		$this->dbh->commit();
				$this->response = $this->alert->alert_toastr("success","$office_role assigned to $full_name Successfully","GSSOTA SAYS")."<script>setTimeout(()=>{
							window.location.reload();
						},2500);</script>";
			 			}else{
			 			$this->response = $this->alert->alert_toastr("error","Something went wrong, Please try again ...","GSSOTA SAYS");
...
public function count_staff_by_gender($gender){
	...
	public function isClassTeacher($staffId){
		$staff_data = self::get_staff_ById($staffId);
		if ($staff_data->staffGrade === "Class Teacher") {
			return true;
		}
	}
